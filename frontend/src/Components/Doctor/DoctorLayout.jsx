...
export default function DoctorLayout() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-[#f9f7fd]">
      {/* SIDEBAR */}
     ...
     <div className="flex-1 p-4 sm:p-6 md:p-8">
       <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
          ...
          <div className="px-4 py-2 rounded-xl bg-white border text-sm text-[#5c4b7a] font-medium w-fit">
            {profile?.name || "Doctor"}
          </div>
        </div>
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8">
      ...
      <div className="bg-white p-6 rounded-2xl border shadow-sm">
        ...
        {recentAppts.length === 0 ? (
         ...
         <div
  key={appt._id}
  className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-4 border rounded-xl mb-2"
>
            <div>
              <p className="font-medium text-[#5c4b7a] break-words">{appt.patientId?.name || "Patient"}</p>
             <p className="text-xs sm:text-sm text-gray-500">{new Date(appt.date).toLocaleDateString()} • {new Date(appt.date).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
            </div>
