...
const api = {
  sendMessage: async (message, sessionId) => {
    ...
    return res.json(); // { success, sessionId, reply }
  },
        ...
        {sessions.map((s) => (
          <div
            ...
            className={`group w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition cursor-pointer ${
              s._id === activeId ? 'bg-[#ede9f7] text-[#5c4b7a] font-medium' : 'text-gray-600 hover:bg-gray-100'
            }`}
            onClick={() => { onSelect(s._id); onMobileClose(); }}
          >
...
const Chatbot = () => {
  ...
  // ── Select session → load messages from backend ───────────────────────────
  ...
  const handleSend = async (textOverride) => {
    ...
    try {
      ...
      // If new session was created, set active and refresh sidebar
      if (!activeSessionId) {
      ...
      setMessages((prev) => [...prev, { text: `⚠️ Error: ${err.message}`, sender: 'bot' }]);
    } finally {
      setIsLoading(false);
  ...
  const handleNewChat = () => {
    setMessages([{ text: "Hello! I'm Saarthi, your healthcare assistant. How can I help you today?", sender: 'bot' }]);
    setInput('');
